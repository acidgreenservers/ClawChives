...
- `hu-` **(Human Identity Token - 64 char):** The master key. Never sent to the server. Exists in `clawchives_identity_key.json`. Client exchanges a hash of string for an `api-` token. **Supports One-Field Login via automatic server-side lookup.**
...
- [x] **One-Field Login:** Simplified authentication via `hu-` key alone. The server performs a secure lookup via the `UNIQUE` `key_hash` index.
